...
%Prepare to save simulation results (Coherent transmission)
userSE_MMSE= zeros(K,nbrOfSetups,1);
userSE_LMMSE= zeros(K,nbrOfSetups,1);
userSE_LS= zeros(K,nbrOfSetups,1);
%Prepare to save simulation results (Non-coherent transmission)
userSE_MMSE_NC= zeros(K,nbrOfSetups,1);
userSE_LMMSE_NC= zeros(K,nbrOfSetups,1);
userSE_LS_NC= zeros(K,nbrOfSetups,1);
...
for m=1:length(M)
    ...
    for n=1:nbrOfSetups
        ...
        %Store SEs for all UEs
        userSE_MMSE_NC(:,n) = SE_CC_MMSE_NC(:);
        userSE_LMMSE_NC(:,n) = SE_CC_LMMSE_NC(:);
...
figure(1);
hold on; box on;
c1=plot(sort(userSE_MMSE(:)),CDFnumbers,'r-');
c2=plot(sort(userSE_LMMSE(:)),CDFnumbers,'r--');
c3=plot(sort(userSE_LS(:)),CDFnumbers,'r-.');
c4=plot(sort(userSE_MMSE_NC(:)),CDFnumbers,'k-');
c5=plot(sort(userSE_LMMSE_NC(:)),CDFnumbers,'k--');
xlabel('SE per UE [bit/s/Hz]');
ylabel('Cumulative Distribution Function (CDF)');
legend([c1, c2,c3,c4,c5],'Coherent trans. with MMSE','Coherent trans. with LMMSE',...
    'Coherent trans. with LS','Non-coherent trans. with MMSE',...
 'Non-coherent trans. with LMMSE/LS','Location','SouthEast');    
