function [SE_CC] = functionTheoreticalCellFreeDLSE_LMMSE_coherent( R,HMean,Dk,M,K,p,tau_p,tau_c,Pset)

%Theoretical DL SE calculation for LMMSE estimator (Coherent transmission)
...
for k = 1:K
    ...
    CCterm1(k)=p*tau_p*trace(Dkm*Omega) ;
    %Non-coherent interference (l=k')
    for l=1:K  
        PsiInv_LMMSE_l = p*tau_p*sum(Rp(:,:,Pset(:,l)),3)  + eyeM;
        Omega_l=(Rp(:,:,l))/PsiInv_LMMSE_l*(Rp(:,:,l));
        CCterm2_p1(k,l)=p*tau_p*trace(Dk(:,:,l)*Rp(:,:,k)*Omega_l);
        
    %Coherent interference (If there is pilot contamination)    
    if any(l==Pset(:,k))  


        CCterm2_p1(k,l)=   CCterm2_p1(k,l) + p*p*tau_p*tau_p*trace(Dk(:,:,l)*R(:,:,k)*R(:,:,k)/PsiInv_LMMSE_l*Omega_l)...
            +2*p*p*tau_p*tau_p*trace(Dk(:,:,l)'*Omega_l/PsiInv_LMMSE_l*Lk(:,:,k)*R(:,:,k))...
            +p*p*tau_p*tau_p*abs(trace(sqrtm(Dk(:,:,l))*Rp(:,:,k)/PsiInv_LMMSE_l*Rp(:,:,l))^2 ...
            -trace(Dk(:,:,l)'*Rp(:,:,k)/PsiInv_LMMSE_l*Rp(:,:,l)*Rp(:,:,k)/PsiInv_LMMSE_l*Rp(:,:,l)));
       ...
       for k=1:K
           SE_CC(k)= prelogFactor*log2(1+ (p*abs(CCterm1(k))^2)/(p*CCterm2(k) - p*abs(CCterm1(k))^2  )  )  ;
       end
